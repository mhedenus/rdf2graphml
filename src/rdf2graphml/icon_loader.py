...
class IconLoader:
    def __init__(self) -> None:
        """
        Initialisiert den IconLoader mit einer eigenen HTTP-Session (Cookies)
        und stellt sicher, dass das Cache-Verzeichnis existiert.
        """
    ...
    def _get_cache_key(self, source: str, target_height: int) -> str:
        """Generates a unique MD5 hash for the URL/path-height combination."""
        unique_string = f"{source}_{target_height}"
        return hashlib.md5(unique_string.encode('utf-8')).hexdigest()

    def _download_with_backoff(self, url: str, max_wait: int = 20) -> Optional[bytes]:
        """
    ...
    def _scale_and_encode(self, image_bytes: bytes, target_height: int) -> Tuple[Optional[str], Optional[int]]:
    ...
    def load_icon_as_base64(self, source: str, is_local: bool = False, target_height: int = 64, base_dir: Optional[Path] = None) -> Tuple[Optional[str], Optional[int]]:
        """
        ...
        if source.startswith("data:image/"):
            ...
            if match:
                ...
                try:
                    ...
                    return self._scale_and_encode(image_data, target_height)
                except Exception as e:
        ...
        if is_local:
            ...
            cache_key = self._get_cache_key(source, target_height)
            ...
            image_data = self._download_with_backoff(source, max_wait=20)
            if not image_data:
                return None, None

        # 3. Process image
        b64_str, width = self._scale_and_encode(image_data, target_height)
